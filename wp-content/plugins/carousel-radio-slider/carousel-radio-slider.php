...
function crs_extract_h3_text($content) {
    preg_match('/<h3[^>]*>([^<]+)<\/h3>/i', $content, $matches);
    return isset($matches[1]) ? trim($matches[1]) : '';
}

function crs_get_featured_image_html($post_id, $size = 'medium') {
    ...
    return wp_get_attachment_image($image_id, $size, false, array('class' => 'alignnone size-' . $size . ' wp-image-' . $image_id));
}
...
function crs_carousel_shortcode($atts) {
    ...
    // If category is explicitly provided, use dynamic mode
    if (!empty($category)) {
        $category_obj = get_term_by('slug', $category, 'category');
        
        if (!$category_obj) {
            return 'Category "' . esc_html($category) . '" not found.';
        }
        ...
        if (empty($posts)) {
            return 'No posts found in category "' . esc_html($category_obj->name) . '".';
        }

        $slide_titles = array();
        $carousel_class = 'crs-carousel crs-carousel-' . sanitize_title($name);
        $html = '<div class="' . esc_attr($carousel_class) . '" data-name="' . esc_attr($name) . '">';
        
        foreach ($posts as $post) {
            $slide_titles[] = $post->post_title;
            $featured_image = crs_get_featured_image_html($post->ID, 'medium');
            
            $html .= '<div class="crs-slide standalone_slide">';
            if (!empty($featured_image)) {
                $html .= $featured_image;
            }
            $html .= '<h3 hidden>' . esc_html($post->post_title) . '</h3>';
            $html .= '</div>';
        }
        
        $html .= '</div>';
        
        // Add slide titles data attribute
        $html = str_replace(
            'data-name="' . esc_attr($name) . '"',
            'data-name="' . esc_attr($name) . '" data-slide-titles=\'' . wp_json_encode($slide_titles) . '\'',
            $html
        );
    ...
    // Otherwise, check for manually created slider
    $sliders = get_option('crs_sliders', array());
    if (isset($sliders[$name])) {
        // Use the manually created slider
        $slides = $sliders[$name];
        if (empty($slides)) {
        ...
        foreach ($slides as $slide) {
            $slide_titles[] = crs_extract_h3_text($slide['content']);
        }

        $carousel_class = 'crs-carousel crs-carousel-' . sanitize_title($name);
        $html = '<div class="' . esc_attr($carousel_class) . '" data-name="' . esc_attr($name) . '" data-slide-titles=\'' . wp_json_encode($slide_titles) . '\'>';
        foreach ($slides as $slide) {
    ...
    // Fall back to dynamic mode using sanitized name as category
    $category_name = sanitize_title($name);
    $category_obj = get_term_by('slug', $category_name, 'category');
    
    if (!$category_obj) {
        return 'Slider not found and category "' . esc_html($category_name) . '" does not exist.';
    }
