...
function cf7rs_get_image() {
    ...
    // Prefer the original/full featured image and provide srcset/sizes
    ...
    if ($thumb_id) {
        ...
        if (!empty($attachment_meta) && is_array($attachment_meta)) {
            $meta['width'] = isset($attachment_meta['width']) ? intval($attachment_meta['width']) : 0;
            $meta['height'] = isset($attachment_meta['height']) ? intval($attachment_meta['height']) : 0;
        }
    }
    ...
    if ($srcset) {
        $response['srcset'] = wp_kses_post($srcset);
    }
    if ($sizes) {
        $response['sizes'] = wp_kses_post($sizes);
    }
    if (!empty($meta)) {
        $response['meta'] = $meta;
    }
    ...
    // Add post link
    $response['post_url'] = esc_url_raw(get_permalink($post->ID));
