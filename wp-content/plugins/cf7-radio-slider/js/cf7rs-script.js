                        ...
                        $.post(cf7rs_ajax.ajax_url, {
                            ...
                            if (resp && resp.success && resp.data) {
                                    ...
                                    if (resp.data.url) {
                                        ...
                                        if (resp.data.srcset) {
                                            $img.attr('srcset', resp.data.srcset);
                                            if (resp.data.sizes) {
                                                $img.attr('sizes', resp.data.sizes);
                                            } else {
                                                $img.attr('sizes', '(max-width: 800px) 100vw, 800px');
                                            }
                                        }

                                        // On load, use responsive sizing (max-width:100%) so image never upscales
                                        $img.on('load', function() {
                                            // Keep original featured image dimensions (don't force scale)
                                            $(this).css({ 'max-width': '100%', 'width': '100%', 'height': 'auto', 'display': 'block' });
                                        ...
                                        var $imgLink = $('<a class="cf7rs-image-link" href="' + postUrl + '"></a>').append($img);
                                        $slide.prepend($imgLink);
                                    }
                                    ...
                                    var $titleLink = $('<a class="cf7rs-title-link" href="' + postUrl + '"></a>').append($titleEl.clone());
                                    ...
                                    if (resp.data.excerpt_html) {
                                        ...
                                        // insert after title link
                                        $titleLink.after($excerptWrap);
                                    }
